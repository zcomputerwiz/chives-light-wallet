...
class TestCATTrades:
    ...
    async def test_cat_trades(self, wallets_prefarm, trusted):
        ...
        assert len(taker_txs) == 3  # One for each: the outgoing CAT, the incoming chives, and the outgoing chives fee
        ...
        assert len(maker_txs) == 1  # The other side will show up as a regular incoming transaction
        assert len(taker_txs) == 2  # One for each: the outgoing chives, the incoming CAT
