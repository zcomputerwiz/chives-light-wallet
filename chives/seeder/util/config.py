...
def patch_default_seeder_config(root_path: Path, filename="config.yaml") -> None:
    ...
    if "seeder" in existing_config:
        print("Chives Seeder section exists in config. No action required.")
        return

    print("Chives Seeder section does not exist in config. Patching...")
    config = load_config(root_path, "config.yaml")
    # The following ignores root_path when the second param is absolute, which this will be
    seeder_config = load_config(root_path, pkg_resources.resource_filename("chives.util", "initial-config.yaml"))
